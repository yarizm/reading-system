...
class ParagraphShareControllerTest {
    ...
    @InjectMocks
    private ParagraphShareController controller;

    @Test
    void shareParagraphRejectsMissingBodyWithoutSavingMessage() {
        when(authContextService.currentUserId(request)).thenReturn(7L);

        Result<?> result = controller.shareParagraph(null, request);
