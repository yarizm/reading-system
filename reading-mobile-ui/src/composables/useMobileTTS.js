...
export function useMobileTTS() {
  ...
  const openAudioPlayer = async (payload) => {
    ...
    try {
      ...
      if (res.data.code !== '200' || !res.data.data?.audioUrl) {
        showFailToast(res.data.msg || '当前听书服务不可用')
      ...
      showFailToast(getRequestErrorMessage(error, '当前听书服务不可用'))
      audioPlayerVisible.value = false
      stopAudioPlayback()
    } finally {
      isAudioLoading.value = false
  ...
  return {
    currentAudio, isGeneratingTts, isChapterPlaying, audioPlayerVisible, audioSharePopupVisible,
